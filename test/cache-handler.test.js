...
function status(overrides = {}) {
  return {
    ...
    recordCount: 123, listedRecordCount: 120, uniqueMcaVersionCount: 40,
...
test('rejects DMs other guilds and non-admin users ephemerally', async () => {
  ...
  for (const input of [
    ...
    interaction({ guildId: '111111111111111111' }),
    interaction({ admin: false }),
  ]) {
...
test('status and update include the Mojang Minecraft manifest cache', async () => {
  ...
  const minecraftService = {
    status: () => ({
      available: true,
      source: 'disk',
      fetchedAt: '2026-07-25T20:30:00.000Z',
      stale: false,
      refreshing: false,
      revision: 4,
      nextRefreshAt: '2026-07-26T02:30:00.000Z',
      lastError: null,
      versionCount: 812,
      releaseCount: 245,
      snapshotCount: 430,
  ...
  assert.match(JSON.stringify(statusInteraction.replies[0]), /Mojang Minecraft versions indexed/);
  assert.match(JSON.stringify(statusInteraction.replies[0]), /812/);

  const updateInteraction = interaction({ subcommand: 'update' });
  await handler(updateInteraction);
  assert.match(JSON.stringify(updateInteraction.edits[0]), /Mojang Minecraft versions indexed/);
});
