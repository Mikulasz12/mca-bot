...
import {
  ...
  normaliseModrinthVersions,
} from '../src/modrinth/catalogue.js';
...
function record({ id, version, minecraft, loader, type = 'release', date, status = 'listed' }) {
  return {
    ...
    status,
    date_published: date,
    files: [{ primary: true, filename: `mca-${loader}-${version}.jar` }],
  };
}
...
test('normalises the Minecraft suffix from MCA version numbers', () => {
  assert.equal(catalogue[0].mcaVersion, '7.7.22');
});

test('verifies MCA Minecraft and loader alignment', () => {
  assert.equal(checkCompatibility(catalogue, {
    mcaVersion: '7.7.23', minecraftVersion: '1.21.1', loader: 'fabric',
  }).status, 'verified');

  assert.equal(checkCompatibility(catalogue, {
    mcaVersion: '7.7.23', minecraftVersion: '26.1.2', loader: 'fabric',
  }).status, 'known-incompatible');

  assert.equal(checkCompatibility(catalogue, {
    mcaVersion: '7.7.23', minecraftVersion: '1.21.1', loader: 'neoforge',
  }).status, 'known-incompatible');
});

test('accepts versions absent from the public catalogue as unknown builds', () => {
  assert.equal(checkCompatibility(catalogue, {
    mcaVersion: '99.0.0-dev', minecraftVersion: '26.1.2', loader: 'neoforge',
...
test('unknown loader does not guess when latest releases differ', () => {
  const latest = findLatestCompatible(catalogue, { minecraftVersion: '1.21.1' });
  assert.equal(latest.status, 'loader-required');
});
